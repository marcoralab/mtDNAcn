...
---
# ROSMAP
```{r setup-rosmap, warning=F, message=F, error=F, echo=FALSE}
# library(knitr)
# library(kableExtra)
library(tidyverse)
library(janitor)
...
rosmap.adl.raw <- readxl::read_xlsx("data/AMPAD_extra/rosmap/dataset_641_long_04-29-2020.xlsx") %>%
  mutate(projid = as.numeric(projid)) %>% 
  select(projid, study, fu_year, iadlsum, katzsum)
...
apoe <- read_tsv("data/AMPAD_extra/rosmap/wgs_apoe.tsv")
...
# Resilience 
resilince <- read_rds("data/AMPAD_extra/rosmap/ROSMAP_resilience_phenotypes_4Shea.rds") %>% 
  as_tibble() %>% 
  mutate(ID = as.numeric(ID))

# Year of Assessment 
yoa <- readxl::read_xlsx("data/AMPAD_extra/rosmap/dataset_889_long_YearOfAssement_11-2020.xlsx") %>%
...
rosmap <- rosmap.pheno_bl %>%
  left_join(resilince, by = c("projid" = "ID")) %>%
  left_join(apoe, by = "projid") %>%
  left_join(rosmap.pheno_long, by = c("projid", "study", "scaled_to")) %>%
  left_join(tot_vol, by = "projid") %>%
  left_join(xcell, by = c("projid" = "ID")) %>%
...
```{r rosmap-rna-numeric, echo=FALSE}
rna_n <- rosmap %>% 
  select(Neurons, Astrocytes, rna_seq_tissue, rna_seq_batch, RIN) %>% 
  inspect_num(.) 
...
rosmap %>% 
  select(Source.Tissue.Type, rna_seq_tissue) %>% 
  mutate(Source.Tissue.Type = as.character(Source.Tissue.Type), 
         rna_seq_tissue = as.character(rna_seq_tissue)) %>%
 gtsummary::tbl_cross(
    row = Source.Tissue.Type, 
    col = rna_seq_tissue, 
    percent = "cell",
  ) 
```
...
path_vars <- c("pmi", "niareagansc", "ceradsc", "braaksc", "gpath", "amyloid", "plaq_d", "plaq_n", "tangles", "nft", "dlbdx", 
...
rosmap_path_crosstab %>% 
  filter(!is.na(niareagansc)) %>%
ggplot(., aes(y = ceradsc, x = braaksc, label = n)) + 
  ...
  labs(title = "ROSMAP: NIA-Regan Diagnosis") + 
  theme(legend.position = "bottom", 
        text = element_text(size=11))

ggsave("/Users/sheaandrews/Downloads/rosmap_dx.pdf", height = 8, width = 8)
...
```{r rosmap-resilience, echo=FALSE}

resilince <- rosmap %>% 
  select(COGRES, GLOBALRES, COGRES_NC, GLOBALRES_NC ) 
...
```{r}
neuroi <- rosmap %>%
  select(study, projid, mri_id, visit, csf, gm, wm, icv,
         apoe_genotype, mtcn_avg, organ, Source.Tissue.Type, 
         msex, pmi, age_bl, age_death, ad_reagan, apoe, cogdx, fu_year_lv) 

```

```{r rosmap-neuroi-numeric, echo=FALSE, message=FALSE, tidy=FALSE}
neuroi_n <- neuroi %>% 
  select(visit, csf, gm, wm, icv) %>%
  inspect_num(.) %>% 
  mutate_if(is.numeric, round, 2) 

neuroi_n %>% 
  select(-hist) %>% 
   DT::datatable(caption = "Variable type: Numeric", rownames = FALSE)
...
rosmap %>%
  mutate(ad_reagan = as_factor(ad_reagan)) %>%
  ggplot(., aes(x = ad_reagan, y = icv, colour = ad_reagan)) + 
    ggbeeswarm::geom_quasirandom(dodge.width=1) +
 facet_grid(. ~ msex) + 
  theme_bw()

rosmap %>%
    filter(!is.na(mtcn_avg), !is.na(icv)) %>%
    mutate(ad_reagan = as_factor(ad_reagan), 
           Source.Tissue.Type = fct_drop(Source.Tissue.Type)) %>%
  ggplot(., aes(x =  mtcn_avg, y = icv, colour = ad_reagan)) + 
    geom_point() + 
    facet_grid(Source.Tissue.Type ~ msex) + 
    theme_bw()
