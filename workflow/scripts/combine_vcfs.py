...
def main(
        participant_data, participants_to_subset, coverage_mt_path, vcf_col_name,
        artifact_prone_sites_path, output_bucket, temp_dir, overwrite,
        minimum_homref_coverage, chunk_size, out_vcf, out_mt
        ):
    ...
    if overwrite == False and hl.hadoop_exists(output_path_mt):
        logger.warn(
    ...
    combined_mt = join_mitochondria_vcfs_into_mt(
        vcf_paths, output_bucket, temp_dir, chunk_size
    )
    ...
    combined_mt = combined_mt.checkpoint(out_mt, overwrite=overwrite)
...
participant_data = snakemake.input['vcf_paths']
participants_to_subset = snakemake.params['participants_to_subset']
coverage_mt_path = snakemake.input['coverage']
vcf_col_name = snakemake.params['VCF_COL_NAME']
artifact_prone_sites_path = snakemake.input['ARTIFACT_PRONE_SITES']
output_bucket = snakemake.params['output_bucket']
temp_dir = snakemake.params['temp_dir']
overwrite = snakemake.params['overwrite']
minimum_homref_coverage = snakemake.params['minimum_homref_coverage']
chunk_size = snakemake.params['chunk_size']

out_vcf = snakemake.output['vcf']
out_mt = snakemake.output['mt']
