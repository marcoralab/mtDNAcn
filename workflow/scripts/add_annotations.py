...
def add_trna_predictions(input_mt: hl.MatrixTable) -> hl.MatrixTable:
    ...
    hl.ReferenceGenome.from_fasta_file("hg37", "raw/reference/human_g1k_v37.fasta.gz", "raw/reference/human_g1k_v37.fasta.fai", x_contigs=["X"], y_contigs=["Y"], mt_contigs=["MT"])
    # add_reference_sequence(hl.get_reference("GRCh37"))
    pon_predictions = pon_predictions.annotate(
        ref=hl.get_sequence(
            "MT", hl.int(pon_predictions.mtDNA_position), reference_genome="hg37"
        )
    )
    pon_predictions = pon_predictions.annotate(
        alt=hl.if_else(
...
def add_rsids(input_mt: hl.MatrixTable) -> hl.MatrixTable:
    ...
    """
    recode = {"NC_012920.1": "chrM"}
    dbsnp_ht = hl.import_vcf(RESOURCES["mt_dbsnp154"],
        reference_genome='GRCh38', force_bgz = False,
        contig_recoding=recode,
        skip_invalid_loci = True,
        min_partitions = True)
    dbsnp_ht = hl.split_multi(dbsnp_ht, permit_shuffle=True)

    input_mt = input_mt.annotate_rows(
        rsid = dbsnp_ht.index_rows(input_mt.locus, input_mt.alleles).rsid
        )
...
def format_vcf(
    ...
    ##VEP version=NA
    ##dbSNP version=NA
    ##age distributions=bin_edges:NA, bin_freq:NA, n_smaller:NA, n_larger:NA
...
def main(args):
    ...
    mt = annotation_descriptions.add_descriptions(
