...
rosmap <- rosmap.raw %>%
  left_join(haplogrep, by = c('WGS_id' = 'SampleID')) %>%
#  left_join(mtdna, by = c('WGS_id' = 'sample')) %>%
  ...
  select(id = WGS_id, study, race, educ, sex = msex, apoe = apoe_genotype, apoe4, cts_mmse30_lv, aod, aod_cat, dx.raw = cogdx, dx, braaksc, ceradsc, abc, npadnc, organ, SourceTissue = Source.Tissue.Type, mtcn_avg, z_mtdnacn, Haplogroup, Quality, macro)
...
**Clinical vs Neuropathological diagnosis in ROSMAP**
```{r, rosmap_npadnc, warning=FALSE, results='asis', echo=FALSE}
rosmap %>% 
  select(dx.raw, npadnc) %>%
  mutate(Clinical.dx = fct_recode(dx.raw, NCI = "1", PROB_MCI = "2", POSS_MCI = "3", 
                                  PROB_AD = "4", PROSS_AD = "5", Other = "6")) %>% 
  count(Clinical.dx, npadnc) %>% 
  pivot_wider(names_from = npadnc, values_from = n) %>% 
  rename('Clinical/NPADNC' = Clinical.dx) %>%
  select('Clinical/NPADNC', low, intermediate, high, 'NA') %>% 
  knitr::kable(.) 
```
