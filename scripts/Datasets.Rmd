...
# write_tsv(haplogrep, 'data/haplogrep/haplogrep_all.txt')
...
phymer <- phymer.files %>% 
  ...
  mutate(study = ifelse(is.na(study), 'DIAN', study), 
         macro = ifelse(str_detect(Haplogroup, "^L"),
                               ...
                               substr(Haplogroup, start = 1, stop = 1)), 
         macro = as.factor(macro))
# write_tsv(phymer, 'data/phymer/phymer_all.txt')
# phymer <- read_tsv('data/phymer/phymer_all.txt')
```
...
mtdna <- list.files(path = "data/mitocalc", pattern = ".txt", full.names = TRUE) %>%
  map(., function(x){
    ...
    tibble(sample = str_remove(str_remove(file, 'data/mitocalc/'), 'MTData.txt'),
           ...
           chrom_used_for_autosomal_coverage = filter(d, X1 == 'chrom_used_for_autosomal_coverage') %>% pull(X2))
...
# write_tsv(mtdna, 'data/mitocalc/mtdna_all.txt')
# mtdna <- read_tsv('data/mitocalc/mtdna_all.txt')
...
# write_tsv(mosdepth, 'data/mosdepth/mosdepth_all.txt')
...
rosmap <- rosmap.raw %>%
  ...
  filter(!is.na(study)) %>%
  select(-Range, -Rank, -scaled_to, -Duplicate, -QC, -Gender_match, -Concordant_with_array, -Concordant_with_RNAseq, -GQN)
...
```{r, rosmap_demo, echo=FALSE, results='asis'}
rosmap %>% 
...
+ WGS sequencing DNA 
  - `Source.Tissue.Type`: Source tissue from where DNA was isolated from 
  - `organ`: collapsed source tissue into organ 
+ Haplogroup
  - `Haplogroup`: Sub-haplogroup determined by haplogrep2 
  - `Quality`: Quality score for haplogroup assignment 
  - `macro`: Macro-haplogroup 
+ mtDNAcn
  - `autosomal_coverage`: average coverage of the autosomal DNA
  - `mt_coverage`: average coverage of the mitochondrial genome
  - `mtcn_avg`: estimated mtDNAcn 
  - `z_mtdnacn`: z-standardized mtDNAcn
+ APOE 
  - `apoe_genotype`: APOE genotypes 
  - `apoe4`: APOE e4 carriers 

```{r, rosmap_gene, echo=FALSE, results='asis'}
rosmap %>% 
  select(Source.Tissue.Type, organ, 
         Haplogroup, macro, Quality, 
         autosomal_coverage, mt_coverage, mtcn_avg, z_mtdnacn, 
         apoe_genotype, apoe4) %>%  
  skim()
```
...
```{r, rosmap_dx, echo=FALSE, results='asis'}
rosmap %>% 
...
```{r, rosmap_path_desc, echo=FALSE, results='asis'}
rosmap %>% 
  select(niareagansc, ceradsc, braaksc, gpath, amyloid, plaq_d, plaq_n, tangles, nft, dlbdx, 
...
### Clinical vs Neuropathological Diagnosis 
